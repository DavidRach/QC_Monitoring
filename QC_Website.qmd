...
```{r}
The3L <- VisualQCSummary(x=Gain_3L)
The4L <- VisualQCSummary(x=Gain_4L)
The5L <- VisualQCSummary(x=Gain_5L)
TheCS <- VisualQCSummary(x=Gain_CS)
```
...
map(.x=InstrumentNames[1], .f=ColorCodeStatus, y=Instruments[1]) %>% bind_rows()
```
